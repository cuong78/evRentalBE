...
public class RentalStationController {
    ...
    public ResponseEntity<ResponseObject> getStationsWithoutAdmin() {
        try {
            List<RentalStationResponse> stations = rentalStationService.getStationsWithoutAdmin();
            return ResponseEntity.ok()
                    .body(ResponseObject.builder()
                            .statusCode(HttpStatus.OK.value())
                            .message("Stations without admin retrieved successfully")
                            .data(stations)
                            .build());
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ResponseObject.builder()
                            .statusCode(HttpStatus.INTERNAL_SERVER_ERROR.value())
                            .message("Failed to retrieve stations: " + e.getMessage())
                            .data(null)
                            .build());
        }
    }

    @PostMapping("/{stationId}/assign-admin/{adminId}")
    public ResponseEntity<ResponseObject> assignAdminToStation(
            @PathVariable Long stationId,
            @PathVariable Long adminId) {
        try {
            RentalStationResponse response = rentalStationService.assignAdminToStation(stationId, adminId);
            return ResponseEntity.ok()
                    .body(ResponseObject.builder()
                            .statusCode(HttpStatus.OK.value())
                            .message("Admin assigned to station successfully")
                            .data(response)
                            .build());
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
    ...
    @PostMapping("/{stationId}/remove-admin")
    public ResponseEntity<ResponseObject> removeAdminFromStation(@PathVariable Long stationId) {
        try {
            RentalStationResponse response = rentalStationService.removeAdminFromStation(stationId);
            return ResponseEntity.ok()
                    .body(ResponseObject.builder()
                            .statusCode(HttpStatus.OK.value())
                            .message("Admin removed from station successfully")
                            .data(response)
                            .build());
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
